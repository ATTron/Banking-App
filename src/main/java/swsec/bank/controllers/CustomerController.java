...
public class CustomerController {
  ...
  // constructor - called when a customer logs in, 
  // so username and password are known, but no other attributes are yet known
  // Assumes that all customers are created a priori; 
  // a new constructor will need to be added to create a new customer that does not previously exist
  ...
  // uses thisRep to lookup customer - if successful, 
  // will load customer attributes into tempCust and mark authenticated
  ...
  public boolean makeDeposit(float amount) throws CustomerNoAccountException, AmountNotVerifiedException, 
         CustomerNotAuthenticatedException, AccountNotFoundException, IOException {
    ...
    if (thisCust.isAuthenticated()) {
      ...
      if (this.VS.confirmAmount(amount)) {
        ...
        // check to make sure that this customer actually has an account, 
        // in which case thisAcct is already populated
  ...
  // returns true if everything goes ok; otherwise throws exceptions
  public boolean makeWithdrawal(float amount) throws CustomerNoAccountException, AmountNotVerifiedException, 
         CustomerNotAuthenticatedException, InsufficientFundsException, AccountNotFoundException, IOException {
  ...
  // returns the balance of this customer's account, 
  // if the customer is authenticated and has an account
  public float getBalance() throws IOException, 
         CustomerNoAccountException, AccountNotFoundException {
    float balance = 0;
  ...
  //returns the AccountController object, otherwise throws an exception
